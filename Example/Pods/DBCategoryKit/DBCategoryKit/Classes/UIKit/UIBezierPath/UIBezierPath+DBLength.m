...
typedef struct DBBezierSubpath {
    CGPoint startPoint;
...
typedef void(^DBBezierSubpathEnumerator)(const CGPathElement *element);

static void db_bezierSubpathFunction(void *info, CGPathElement const *element) {
    DBBezierSubpathEnumerator block = (__bridge DBBezierSubpathEnumerator)info;
    block(element);
}

@implementation UIBezierPath (DBLength)

#pragma mark - Internal

- (void)enumerateSubpaths:(DBBezierSubpathEnumerator)enumeratorBlock
{
    CGPathApply(self.CGPath, (__bridge void *)enumeratorBlock, db_bezierSubpathFunction);
}
...
- (void)extractSubpaths:(DBBezierSubpath*)subpathArray
{
...
- (CGPoint)pointAtPercent:(CGFloat)t ofSubpath:(DBBezierSubpath)subpath {
...
- (CGFloat)db_length {
    ...
    DBBezierSubpath subpaths[subpathCount];
...
- (CGPoint)db_pointAtPercentOfLength:(CGFloat)percent {
    ...
    DBBezierSubpath subpathContainingPoint;
    for (NSUInteger i = 0; i < subpathCount; i++) {
