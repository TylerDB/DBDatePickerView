...
static CIDetector *db_detector;

@implementation UIImageView (DBBetterFace)

void db_hack_uiimageview_bf(){
    Method oriSetImgMethod = class_getInstanceMethod([UIImageView class], @selector(setImage:));
    Method newSetImgMethod = class_getInstanceMethod([UIImageView class], @selector(db_setBetterFaceImage:));
    method_exchangeImplementations(newSetImgMethod, oriSetImgMethod);
}

- (void)db_setBetterFaceImage:(UIImage *)image{
    [self setDb_needsBetterFace:image != nil];
    if (![self db_needsBetterFace]) {
        return;
    }
    
    [self db_faceDetect:image];
}

char nbfKey;
- (void)setDb_needsBetterFace:(BOOL)needsBetterFace{
...
char detectorKey;
- (void)setDb_detector:(CIDetector *)detector{
    objc_setAssociatedObject(self,
                             &detectorKey,
                             detector,
                             OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)db_faceDetect:(UIImage *)aImage
{
    ...
    dispatch_async(queue, ^{
        ...
        if (db_detector == nil) {
            NSDictionary  *opts = [NSDictionary dictionaryWithObject:[self db_fast] ? CIDetectorAccuracyLow : CIDetectorAccuracyHigh
                                                              forKey:CIDetectorAccuracy];
            db_detector = [CIDetector detectorOfType:CIDetectorTypeFace
                                             context:nil
                                             options:opts];
        }
        
        NSArray* features = [db_detector featuresInImage:image];
        
        if ([features count] == 0) {
            NSLog(@"no faces");
            dispatch_async(dispatch_get_main_queue(), ^{
                [[self db_imageLayer] removeFromSuperlayer];
            });
        } else {
            NSLog(@"succeed %lu faces", (unsigned long)[features count]);
            [self db_markAfterFaceDetect:features
...
-(void)db_markAfterFaceDetect:(NSArray *)features size:(CGSize)size{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        CALayer *layer = [self db_imageLayer];
