...
static CIDetector * _db_faceDetector;

@implementation UIImageView (DBFaceAwareFill)

+ (void)initialize
{
    _db_faceDetector = [CIDetector detectorOfType:CIDetectorTypeFace
                                          context:nil
...
- (void)db_faceAwareFill {
    ...
    CGRect facesRect = [self db_rectWithFaces];
    if (facesRect.size.height + facesRect.size.width == 0)
        return;
    self.contentMode = UIViewContentModeTopLeft;
    [self db_scaleImageFocusingOnRect:facesRect];
}

- (CGRect)db_rectWithFaces {
    // Get a CIIImage
    ...
    CIDetector* detector = _db_faceDetector;
...
- (void)db_scaleImageFocusingOnRect:(CGRect) facesRect {
