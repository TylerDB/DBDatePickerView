...
@implementation NSObject (DBEasyCopy)
    ...
    while (instanceClass != [NSObject class]) {
        ...
        for (int i = 0; i < propertyListCount; i++) {
            ...
            if (propertyName && !readonly) {
                ...
                BOOL flag = [NSObject db_isNSObjectClass:propertyValueClass];
                if (flag) {
                    if ([propertyValue conformsToProtocol:@protocol(NSCopying)]) {
                        ...
                        [copyValue db_easyDeepCopy:propertyValue];
                        [self setValue:copyValue forKey:propertyName];
                    }
                }else{
                    [self setValue:propertyValue forKey:propertyName];
...
+ (BOOL)db_isNSObjectClass:(Class)clazz{
    ...
    if (flag) {
        ...
        if (!superClass) {
            ...
            return  [NSObject db_isNSObjectClass:superClass];
        }
    }
}
