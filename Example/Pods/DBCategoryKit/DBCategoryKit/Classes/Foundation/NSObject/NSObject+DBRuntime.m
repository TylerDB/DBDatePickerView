...
BOOL db_method_swizzle(Class klass, SEL origSel, SEL altSel)
{
    if (!klass)
        return NO;
...
void db_method_append(Class toClass, Class fromClass, SEL selector)
{
...
void db_method_replace(Class toClass, Class fromClass, SEL selector)
{
    if (!toClass || !fromClass || ! selector)
        return;
...
@implementation NSObject (DBRuntime)

+ (void)db_swizzleMethod:(SEL)originalMethod withMethod:(SEL)newMethod
{
    db_method_swizzle(self.class, originalMethod, newMethod);
}

+ (void)db_appendMethod:(SEL)newMethod fromClass:(Class)klass
{
    db_method_append(self.class, klass, newMethod);
}

+ (void)db_replaceMethod:(SEL)method fromClass:(Class)klass
{
    db_method_replace(self.class, klass, method);
}

- (BOOL)db_respondsToSelector:(SEL)selector untilClass:(Class)stopClass
{
    return [self.class db_instancesRespondToSelector:selector untilClass:stopClass];
}
...
{
    return [self.superclass db_instancesRespondToSelector:selector untilClass:stopClass];
}
