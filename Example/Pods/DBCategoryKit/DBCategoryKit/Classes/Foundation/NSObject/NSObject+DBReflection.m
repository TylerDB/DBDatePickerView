...
@implementation NSObject (DBReflection)
...
{
    return [[self class] db_propertyKeys];
}
...
{
    return [[self class] db_propertiesInfo];
}
...
+ (NSArray *)db_propertiesInfo
{
    ...
    for (int i = 0; i < propertyCount; i++)
    {
        [propertieArray addObject:({
            ...
            NSDictionary *dictionary = [self db_dictionaryWithProperty:properties[i]];
...
+ (NSArray *)db_propertiesWithCodeFormat
{
    ...
    NSArray *properties = [[self class] db_propertiesInfo];
...
- (NSArray *)db_methodListInfo{
    ...
    for (int i = 0; i < count ; i++)
    {
        ...
        for (int index=0; index<argumentsCount; index++) {
            ...
            [arguments addObject:[[self class] db_decodeType:arg]];
        }
        
        NSString *returnTypeString =[[self class] db_decodeType:returnType];
        NSString *encodeString = [[self class] db_decodeType:encoding];
...
- (NSDictionary *)db_protocolList{
    return [[self class]db_protocolList];
}
...
+ (NSArray *)db_instanceVariable
{
    ...
    NSMutableArray *result = [NSMutableArray array];
    for (int i = 0; i < outCount; i++) {
        NSString *type = [[self class] db_decodeType:ivar_getTypeEncoding(ivars[i])];
...
#pragma mark -- help
+ (NSDictionary *)db_dictionaryWithProperty:(objc_property_t)property
{
...
+ (NSString *)db_decodeType:(const char *)cString
{
    ...
    } else
    {
        if ([[result substringToIndex:1] isEqualToString:@"^"]) {
            result = [NSString stringWithFormat:@"%@ *",
                      [NSString db_decodeType:[[result substringFromIndex:1] cStringUsingEncoding:NSUTF8StringEncoding]]];
        }
    }
    return result;
}
