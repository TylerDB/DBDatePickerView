...
@implementation NSInvocation (DBBlock)
...
{
    NSInvocation* invocation = [NSInvocation db_invocationWithBlock:block];
...
+ (id)db_doInstanceMethodTarget:(id)target selectorName:(NSString *)selectorName args:(NSArray *)args
{
    ...
    [invocation db_setArgumentsWithArray:args];
...
- (void)db_setArgumentsWithArray:(NSArray *)array
{
    if (nil != array && array.count > 0) {
        ...
        if (array.count != numargs) {
            ...
            [self db_setArgumentWithObject:arg atIndex:0];
        }else{
            for (NSUInteger i = 0; i < numargs; i++) {
                [self db_setArgumentWithObject:array[i] atIndex:i];
            }
        }
    }
}
