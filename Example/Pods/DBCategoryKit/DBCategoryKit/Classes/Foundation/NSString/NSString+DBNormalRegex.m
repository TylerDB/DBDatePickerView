...
@implementation NSString (DBNormalRegex)

#pragma mark - 正则相关
- (BOOL)db_isValidateByRegex:(NSString *)regex{
...
//手机号分服务商
- (BOOL)db_isMobileNumberClassification{
    //运营商手机号段划分
    ...
    if (([self db_isValidateByRegex:CM])
        || ([self db_isValidateByRegex:CU])
        || ([self db_isValidateByRegex:CT])
        || ([self db_isValidateByRegex:PHS])
        || ([self db_isValidateByRegex:HK]))
    {
        return YES;
    }
    else
...
- (BOOL)db_isMobileNumber{
    ...
    BOOL ret1 = [self db_isValidateByRegex:mobileRegex];
    return ret1;
}

//邮箱
- (BOOL)db_isEmailAddress{
    NSString *emailRegex = @"[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,4}";
    return [self db_isValidateByRegex:emailRegex];
}

//身份证号
- (BOOL)db_simpleVerifyIdentityCardNum
{
    NSString *regex2 = @"^(\\d{14}|\\d{17})(\\d|[xX])$";
    return [self db_isValidateByRegex:regex2];
}
...
- (BOOL)db_isCarNumber{
    ...
    return [self db_isValidateByRegex:carRegex];
}

- (BOOL)db_isMacAddress{
    NSString * macAddRegex = @"([A-Fa-f\\d]{2}:){5}[A-Fa-f\\d]{2}";
    return  [self db_isValidateByRegex:macAddRegex];
}

- (BOOL)db_isValidUrl
{
    NSString *regex = @"^((http)|(https))+:[^\\s]+\\.[^\\s]*$";
    return [self db_isValidateByRegex:regex];
}

- (BOOL)db_isValidChinese;
{
    NSString *chineseRegex = @"^[\u4e00-\u9fa5]+$";
    return [self db_isValidateByRegex:chineseRegex];
}

- (BOOL)db_isValidPostalcode {
    NSString *postalRegex = @"^[0-8]\\d{5}(?!\\d)$";
    return [self db_isValidateByRegex:postalRegex];
}

- (BOOL)db_isValidTaxNo
{
    NSString *taxNoRegex = @"[0-9]\\d{13}([0-9]|X)$";
    return [self db_isValidateByRegex:taxNoRegex];
}
...
#pragma mark - 算法相关
//精确的身份证号码有效性检测
+ (BOOL)db_accurateVerifyIDCardNumber:(NSString *)value {
