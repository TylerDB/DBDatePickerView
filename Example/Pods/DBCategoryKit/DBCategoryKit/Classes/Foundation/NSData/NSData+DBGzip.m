...
@implementation NSData (DBGzip)

static void *db_libzOpen()
{
    static void *libz;
...
{
    if (self.length == 0 || [self db_isGzippedData])
    {
        return self;
    }
    
    void *libz = db_libzOpen();
...
{
    return [self db_gzippedDataWithCompressionLevel:-1.0f];
}
